export interface IWHOReference {
    age: number;
    gender: "male" | "female";
    meanBMI: number;
    sd: number;
...
const WHO_BMI_REFERENCES: IWHOReference[] = [
    // Laki-laki
...
const calculateZScore = (references: IWHOReference[], payload: Pick<IWHOReference, "age" | "gender" | "meanBMI">) => {
...
export const determineNutritionStatus = (bmi: number) => {
    if (bmi < 17) {
        return {
            statusId: 1,
            status: 'KURUS',
